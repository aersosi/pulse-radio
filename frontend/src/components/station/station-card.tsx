...
export default function StationCard({station}: { station: Station }) {
    return (
        <Link href={`/station/${station.id}`} className="block">
            ...
            >
                <div
                    className="transition rounded-xl py-6 border hover:border-green-500/50 hover:bg-green-500/5  shadow-sm hover:shadow-green-500/50">
                    <CardHeader className="h-[32px]">
                        <CardTitle>{station.name}</CardTitle>
                    </CardHeader>
                    <CardContent className="py-[var(--step-8-24)]">
                        <div className="relative fludidStationLogo mx-auto">
                            <Image
