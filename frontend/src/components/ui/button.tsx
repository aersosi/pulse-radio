...
const pulseText = "hover:text-primary"
const pulseOutline = "hover:border-primary/30"
const destructiveBG = "hover:bg-red-500/10"
const destructiveText = "hover:text-red-600"
const destructiveOutline = "hover:border-red-600"
  ...
  "cursor-pointer inline-flex items-center justify-center gap-2 whitespace-nowrap rounded-md text-sm font-medium transition-colors disabled:pointer-events-none disabled:opacity-50 [&_svg]:pointer-events-none [&_svg:not([class*='size-'])]:size-4 shrink-0 [&_svg]:shrink-0 outline-none focus-visible:border-ring focus-visible:ring-ring/50 focus-visible:ring-[3px] aria-invalid:ring-destructive/20 dark:aria-invalid:ring-destructive/40 aria-invalid:border-destructive",
  {
    variants: {
      variant: {
        ...
        outlinePulse:
          `border border-input bg-background shadow-xs ${pulseBG} ${pulseText} ${pulseOutline}`,
        outlineDestructive:
          `border border-input bg-background shadow-xs ${destructiveBG} ${destructiveText} ${destructiveOutline}`,
        secondary:
          `bg-secondary text-secondary-foreground shadow-xs hover:bg-secondary/80`,
        transparentPulse: `${pulseText}`,
        transparentDestructive: `${destructiveText}`,
        ghost: `hover:bg-accent hover:text-accent-foreground`,
        ghostPulse: `${pulseBG} ${pulseText}`,
        ghostDestructive: `${destructiveBG} ${destructiveText}`,
        link: `text-primary underline-offset-4 hover:underline`,
      },
      size: {
        default: `h-9 px-4 py-2 has-[>svg]:px-3`,
        xs: `h-7 rounded-sm gap-1.5 px-2 text-xs`,
